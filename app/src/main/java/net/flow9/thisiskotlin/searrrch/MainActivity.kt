...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.cvBtnFrag1.setOnClickListener {
...
//                binding.cvBtnFrag1.foreground = ContextCompat.getDrawable(this, R.drawable.apply_ripple_effect)
        ...
        binding.cvBtnFrag2.setOnClickListener { binding.apply {
//                Toast.makeText(this@MainActivity, "우 버튼 테스트", Toast.LENGTH_SHORT).show()
                if (cvBtnFrag2.alpha == 0.5f) {
                    val fadeIn = ObjectAnimator.ofFloat(cvBtnFrag1, "alpha", 1f, 0.5f)
                    fadeIn.duration = 200
                    fadeIn.start()

                    val fadeout = ObjectAnimator.ofFloat(cvBtnFrag2, "alpha", 0.5f, 1f).apply {
                        duration = 200
                        start()
                    }
                }
